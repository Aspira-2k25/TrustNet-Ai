...
from services.scan_management.app.routers.scan_router import router as scan_router

logger = get_logger(settings.SERVICE_NAME)

@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("Initializing database tables for Scan Management Service...")
    await init_db()
    await publisher.start()
    logger.info("Scan Management Service initialized successfully.")
    yield
    await publisher.stop()
    logger.info("Scan Management Service shutting down...")

app = FastAPI(
    title="TrustNet Scan Management Service",
    description="Scan Lifecycle and Orchestration Management Service for TrustNet AI",
    version="1.0.0",
    lifespan=lifespan
)
...
async def health_check():
    ...
    return APIResponse(
        data={"status": "ok", "service": "scan_management_service"},
        meta=ResponseMeta(request_id=request_id)
    )
