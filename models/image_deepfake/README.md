...
| **13** | **LM Studio Local Vision Reasoning Engine** | LM Studio / OpenAI-compatible Vision API (`qwen3-vl-4b-thinking`) | Multimodal semantic reasoning & debrief | Analyzes fine texture, lighting anomalies, and anatomy boundaries via local vision LLM. Strips `<think>` blocks, enforces strict JSON schema, downscales to <150KB JPEG to avoid CPU latency, and contributes 0.18–0.20 vote. | `inference/lm_studio_vision_client.py` |
| **14** | **Provenance & AI Metadata Forensics** | Pillow / PyExif / XMP Parser | Cryptographic & software provenance | Scans EXIF tags, PNG info chunks, and generation parameter dictionaries for 50+ known AI signatures (Midjourney, DALL-E, SDXL, Flux, ComfyUI, etc.) with hard priority override. | `forensics/metadata_analyzer.py` |
| **15** | **Pixel-Level Watermark Icon Scanner** | OpenCV Contour & Convexity Defects | Visual provenance icon detection | Examines image corners for platform watermark glyphs using convexity defect pointedness analysis and aspect solidity filtering. | `forensics/watermark_analyzer.py` |
| **16** | **Social Re-Compression & DCT Grid Analyzer** | NumPy 8x8 DCT Boundary Matrix | Compression history & heuristic scaling | Computes blockiness ratio across 8x8 DCT grid lines. When social recompression (e.g. WhatsApp) is detected, downweights fragile heuristics and shifts authority to robust neural/metadata signals. | `forensics/recompression_analyzer.py` |
